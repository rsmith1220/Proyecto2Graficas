...
class Raytracer(object):
    ...
    def cast_ray(self, orig, dir, sceneObj = None, recursion = 0):
        ...
        if material.matType == OPAQUE:
            ...
            reflect = reflectVector(intersect.normal, np.array(dir) * -1)
            reflectOrig = matesRS.add(intersect.point, bias) if outside else np.subtract(intersect.point, bias)
            ...
            if kr < 1:
                ...
                refractOrig = matesRS.subtract(intersect.point, bias) if outside else np.add(intersect.point, bias)
    ...
    def glRender(self):
        ...
        for y in range(self.vpY, self.vpY + self.vpHeight + 1, STEPS):
            for x in range(self.vpX, self.vpX + self.vpWidth + 1, STEPS):
                ...
                direction = matesRS.normal2(direction)
